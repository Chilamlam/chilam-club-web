...
- 管理员账号在 Supabase `users` 表里手工建立，**账号与口令一律不写进本仓库**
  （本仓库是 public，写进来等于公开发布）。
- 口令按 `database.hash_password()` 的 PBKDF2-HMAC-SHA256（10 万次迭代 + 随机 salt）存储。
- 历史教训：本节曾直接写出初始口令，且 `database.py` 里配了一条
  「特定邮箱 + 特定明文口令直接放行」的兜底分支 —— 两者叠加等于把后台管理权公开。
  2026-09-03 已删除该分支并清理此处口令；**因 git 历史无法撤回，该口令必须视为已泄露、
  只能靠轮换处置**。
