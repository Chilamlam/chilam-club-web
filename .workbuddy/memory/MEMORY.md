...
- **下游「保护逻辑生效」只能证明下游没错，证不到上游没出事**（09-01 我据此把真故障判成正常，被用户当场纠正）。定位静默失败必须回溯到最上游那一步。
- **AST 断言必须认对人**：`第一个 st.info` 抓到的是开头「暂无数据」早退分支，不是免责声明 → 按实参文本特征认（含「不构成投资建议」），并补反向验证证明过滤器不恒真。
- **用例本身要有区分度**：ETF「写锚定日 vs 写系统日期」若锚定日恰等于今天，两种实现产物一模一样、断言退化恒真 → 用远期日历，并加一条「锚定日 != 系统日期」的元断言。
- **反向验证别 monkey-patch 整个函数**（那是换实现，证不到目标那一行）→ 改源码字符串再 exec（`_load_patched()`），**锚点失配立即抛错**，锚点空转比断言恒真更糟。

## 跑批可靠性铁律（09-01 四班全灭换来的）
- **日期锚定以「数据是否已发布」为准，不以交易日历为准**。日历只答「这天开不开市」。Actions cron 是 best-effort（实测延迟 1.2h~9.2h，历史最大 +12h），过零点后日历首位=当天而行情未发布 → 整步空转。正确范式见 `daily_breakout.py::get_latest_trade_date`；`MAX_ANCHOR_BACK=3` 逐日探测，超限报错不静默用旧数据。**回退锚定日后各窗口对照日必须 `cal[anchor_i+n]`**，否则窗口悄悄少一天、外观完全正常。
- **写盘日期必须锚定交易日，绝不用系统日期**（`datetime.now()`）。这与锚定是两条独立的线：锚定对了、写盘用系统日期照样造出不存在的交易日（commit `9cf6aec` 把周六 2026-08-29 写进 `strong_etfs.csv`）。
- **装饰性字段绝不挡在主产物前**。判据：该字段缺失会不会让主产物失去意义。两阶段落盘 —— 主产物先 `to_csv`，装饰字段后补且带独立时间预算（run 341 已算好 102 只榜单因题材接口挂住被超时杀掉，磁盘零字节）。线程池抓取用 `shutdown(wait=False, cancel_futures=True)` 且不能用 `with`（`with` 退出必 join）。
- **退出码必须如实 + 监控判据逐产物覆盖 + 结论由机器给**。`main_job` 恒 `return None` = 退出码 0 = 汇总 `[OK]`；看门狗探针漏产物 = 判「已就位」；「打印了原始日期让人肉眼比对」不算监控。`run_daily.check_date_consistency()` 取批内最大日期当基准（**刻意不引交易日历**：过零点/周末/节假日必误报；同批产物本该同日，能精确抓「某步单独失败」），落后者点名 + `::warning::`。
- **修一个坑先横向 grep 同类实现**（`grep -n "cal_date|trade_cal|loc\[0" daily_*.py`）。同仓曾三套实现同一问题、两对两错；ETF 的同坑位与那个周六日期 bug 都是这一步捞出来的。
