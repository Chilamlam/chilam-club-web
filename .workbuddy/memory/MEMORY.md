...
- **⚠️ 未修的同类坑：`page_fibonacci.py`（📏 黄金分割预测页）是独立实现**，第 32 行仍是老的 `"sh" if code.startswith(("6","5","9")) else "sz"` 硬编码 + 老 `fqkline` 端点 → 北交所取不到数（且静默只给 1 根）、`000831` 类撞号无条件判深市。它没复用 `resolve_symbol`/`resolve_candidates`。修的方向是**复用行情页的解析与候选**，别在这页再抄一份前缀规则。
