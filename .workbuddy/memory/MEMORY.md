...
- **API 推送必须自己归一化 CRLF→LF**（脚本已内置 `_to_lf()`）：本机 `core.autocrlf=true` 且仓库无 `.gitattributes`，git 存 blob 是 LF、工作区检出是 CRLF。绕过 git 直接写 blob 若不归一化，远端那几个文件就变成 CRLF → 下次 fetch 后被 git 判成**整份已修改**，之后每次提交都翻一遍全文件行尾，diff 全是噪点（9/1 已修，并清洗历史遗留的 8 个文件；**查行尾用仓库 tarball 一次扫完，别逐个 blob 请求**）。通用教训：绕过 git 写仓库时，git 在提交路径上做的规范化，绕过者必须自己补上。
