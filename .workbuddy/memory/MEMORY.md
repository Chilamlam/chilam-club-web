...
- **反向验证九铁律**：①造错本身先验有效（smartbox 忽略未知 URL 参数 → 须换不存在的 host；**等价改动也算没造**：`minutes=RUNTIME_MIN+MARGIN_MIN` 造成 `minutes=53` 而 18+35 恰好 53 → 行为逐位相同）；②探针失败只走 `bad()` 唯一出口（自己 print FAIL 会漏抓）；**`expect` 须在 `[FAIL]` 行里命中而非全量输出**——`ck(cond,msg)` 成败都打印同一句 msg，「别的断言挂了+msg 出现在 PASS 行」会被算成 caught；期望关键字要取**失败分支才打印**的文案（`try: f(); bad("必须抛错") except: ck(True,"带列名")` 造错后走 `bad()`，写「带列名」永远命中不了）；③断言别被无关数据喂饱（PRESETS 本就有「中国稀土」按钮 → 搜索挂了照样过）；④别 monkey-patch 整函数（那是换实现）→ 改源码字符串，锚点先验唯一、失配即报「造错未生效」；⑤断言要放在**能验出它**的环境（stub 掉 `cache_data` 后「空结果不缓存」恒真；`end` 用固定过去日期验不出「end=当天才丢末根」）；⑥按 tag 跑子集时 tag 写错=零用例=fail 0 → 须判「没抓住」；⑦**参照物不能与被测对象共享故障模式**（拿「同一函数的短区间」当参照，造错后两边一起错、对比恒等=假断言）→ 参照须独立通道，通道自己挂了报「本次什么都没验到」而非 PASS；⑧**fixture 必须真能走到被测分支**：拿 `any(_env(k) for k in (...))` 当「无短路遍历」的反例，而生成器表达式在 AST 里是 `GeneratorExp`、进不了 `ast.For` → 检测器怎么改都返回 `[]`，断言永不失败；⑨**复合条件的每个子条件都要有专属 fixture**：`if _short and _reads:` 只有两条 fixture 时删掉 `_reads` 仍全绿 → 靠「对探针自己造错」才发现。**副作用型被测对象**（会派发跑批/发消息/写库）：所有触达副作用的用例必须把副作用函数换成计数器并**显式断言不该触发处为 0 次**，「跑完没出事」不是证据。
- **公开仓库 + 提交产物 = 潜在 PII 泄露**（09-03 险中）：`data/` 随 Actions 提交而仓库是 public，任何落盘产物写入投递凭据（WxPusher UID，配环境里的 app_token 即可冒名推送）或邮箱（出现即等于「此人是付费会员」）都是公开泄露。去重账本只需回答「这个目标今天收过吗」，查询时目标已知 → **存 HMAC 摘要键，永不存原文**；**邮箱可枚举，裸 sha256 = 「输入邮箱查是否会员」的公开查询接口，必须带密钥**；密钥取首个已配 secret 且**绝不落盘**，一个都没有时**放弃记账**（宁可多推一次）——**绝不退化成硬编码兜底盐，盐在公开仓库里就是公开的，假脱敏比不脱敏更危险**。不含个人信息的群维度标记（`__wecom__`）可留原文便于人工阅读。新增落盘产物前先问一句「这个字段公开了会怎样」。
- **跨 run 产物必须验「它真能活到下一班」**：账本被 `.gitignore` 挡掉或 yml 漏 `git add`，则每班都读到空账本 → 去重全程空转、天天重复推，而日志照样打印「账本已更新」。判据用外部事实：`git check-ignore` 退出码（**并拿一个已知被忽略的路径校准探法**，否则该断言恒过）+ yml 提交步骤正则（注释不算）。
- **「名字在文件里出现过」不等于「生效」**：yml 注释里正当写满 secret 名，`name in wf` 于是把注释当点名放过真·漏点名 → 须严格匹配映射行 `NAME: ${{ secrets.NAME }}`（顺带能抓「映射到错误 secret」）。反之**判据过度报警同样有害**（训练人忽略自检）：回落链 `for k in (A,B,C): if v: return v` 上有人点名即不会静默失效 → 用 AST 识别「元组遍历+短路返回」豁免其余成员，但**链上一个都没点名时照旧全员报缺**，且**另有直接读取处的名字永不豁免**。扫描文件集要按**读取处**而非目录约定圈定（`WXPUSHER_APP_TOKEN` 真实读取在 `wxpusher.py`，只扫 `daily_*.py` 会漏）。
- **判「文件被改了」看内容 sha，别只看 `git status` 的脏标记**：反验脚本重写文件后 index 的 stat 缓存过期，status 显示 ` M` 而 `git hash-object` 与 `HEAD:<path>` 同 sha、`git diff --exit-code` 退出 0。
